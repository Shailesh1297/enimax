...
window.addEventListener("videoDurationChanged", () => {
    try {
        window.parent.apiCall("POST", { "username": username, "action": 2, "name": data_main.nameWSeason, "nameUm": data_main.name, "ep": data_main.episode, "duration": Math.floor(a.vid.duration), "cur": location.search }, (x) => { });
    }
    catch (err) {
        console.error(err);
    }
